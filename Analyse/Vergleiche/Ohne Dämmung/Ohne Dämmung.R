...
tabelle_250_holz <- read_excel("./Analyse/250Holz/Kombiniert.xlsx") %>%
...
ggplot(alle_tabellen, aes(x = Frequency, y = Differenz, color = Tabelle)) +
  ...
  scale_color_manual(
    values = c("170 Holz" = "red", "250 Holz" = "darkgreen", "306 Holz" = "blue")  # Farben anpassen
  ) +
  labs(
    title = "Dämpfung Holz ohne Dämmung",  # Titel der Grafik

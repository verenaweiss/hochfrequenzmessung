...
library(openxlsx)
tabelle1 <- read.xlsx("C:\\Users\\veren\\OneDrive\\Dokumente\\Holztechnikum\\Diplomarbeit\\Analyse\\170HolzDämmung\\170HolzDämmung_neu.xlsx")
...
write_xlsx(kombinierte_tabelle, "C:/Users/veren/OneDrive/Dokumente/Holztechnikum/Diplomarbeit/Analyse/170HolzDämmung/Kombiniert.xlsx")


library(ggplot2)

# 3. Excel-Datei einlesen
daten <- read_excel("C:/Users/veren/OneDrive/Dokumente/Holztechnikum/Diplomarbeit/Analyse/170HolzDämmung/Kombiniert.xlsx")
...
daten$`Dämpfung [dB]` <- as.numeric(daten$`Dämpfung [dB]`)

# 6. Grafik erstellen
grafik <- ggplot(daten, aes(x = Frequency, y = `Dämpfung [dB]`)) + 
  geom_line(color = "blue", size = 0.5) +  # Linie für den Verlauf
  labs(
    title = "Dämpfung 17 cm Holz mit Dämmung",  # Titel der Grafik
